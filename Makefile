...
install:
	...
	@echo 'TAF_PATH    :=/Volumes/mage/tool/taflib/taf-'${TAF_VERSION} > makefile.taf.tmp
	...
	if [  -d ../tafjce ]; then cp -rf script/release ../tafjce; fi
	if [  -d ../tafjce/release ]; then ../tafjce/release/clean_obj.sh; fi
	if [  -d ../tafjce/release ]; then ../tafjce/release/release_all.sh; fi
